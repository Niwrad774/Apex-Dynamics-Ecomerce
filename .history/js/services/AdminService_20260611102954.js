export class AdminService {
    ...
    getDashboardMetrics() {
        ...
        // 1. Cálculo de Ingresos Totales (Suma de todas las órdenes)
        const totalRevenue = orders.reduce((acc, order) => acc + parseFloat(order.total), 0);

        // 2. Métrica de los 3 Productos más vendidos (Simulación basada en cantidades o rating alto)
        const topProducts = [...products]
            .sort((a, b) => b.rating - a.rating)
            .slice(0, 3)
            .map(p => ({ title: p.title, rating: p.rating }));

        // Retorna un DTO de métricas limpio listo para ser consumido por la Vista
        return {
