...
document.addEventListener("DOMContentLoaded", () => {
    ...
    DOM.productForm.addEventListener("submit", (e) => {
        ...
        if (productDto.id) {
            ...
            // PUNTO DE CONEXIÓN: Flujo de Creación
    ...
    // 5. CONEXIÓN DE CAMBIO DE ESTADO LOGÍSTICO (Dropdown de Envío)
    DOM.salesTableBody.addEventListener("change", (e) => {
        if (e.target.classList.contains("order-status-select")) {
            const orderId = e.target.dataset.id;
            const newStatus = e.target.value;
            
            // PUNTO DE CONEXIÓN: Al cambiar el combo, impacta directo la lógica y recalcula
    ...
    // DISPARADOR INICIAL: Conecta y dibuja el estado de la aplicación al abrir la página
    refreshAdminPanel();
});